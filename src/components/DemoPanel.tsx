...
export default function DemoPanel() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setRunners((r) => r + (Math.random() > 0.7 ? 1 : 0));
      setRevenue((rev) => rev + Math.floor(Math.random() * 5000));
    }, 2000);
  ...
  return (
    <div className="h-full w-full bg-gradient-to-br from-[#0f0f0f] to-[#1a1a1a] p-6 text-white flex flex-col">
      ...
      <div className="mt-auto pt-6">
        <div className="h-36 bg-black/40 rounded-lg flex items-center justify-center text-zinc-500 text-sm">
